...
func NewEcrypted(session gockle.Session, tableName string) *hashedPasswordHandler {
...
func (h *hashedPasswordHandler) Compare(ctx context.Context, oidcIssuer, oidcSubject, password string) (ok bool, errUC *types.CommonError) {
...
func (h *hashedPasswordHandler) Set(ctx context.Context, oidcIssuer, oidcSubject, password string) (errUC *types.CommonError) {
	encrypted, err := bcrypt.GenerateFromPassword([]byte(password), bcrypt.MaxCost)
	if err != nil {
